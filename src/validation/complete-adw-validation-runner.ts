...
import { ADWQualityGates } from './ADWQualityGates.js';
 ...
 */
async function runCompleteADWValidation(): Promise<CompleteValidationRunReport> {
  ...
  try {
    ...
    const gate0Execution = await qualityGates.executeGate(
      'gate-0-zero-drift',
      workflowId,
      evidenceTracker,
      gate0Data
    ...
    const gate1Execution = await qualityGates.executeGate(
      'gate-1-build',
      workflowId,
      evidenceTracker,
      gate1Data
    ...
    const report: CompleteValidationRunReport = {
      workflowId,
...
function calculateEvidenceQualityBreakdown(results: GateValidationSummary[]): EvidenceQualityBreakdown {
