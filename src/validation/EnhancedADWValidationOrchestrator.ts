...
import { ADWQualityGates, GateExecution, QualityGateType } from './ADWQualityGates.js';
import { EvidenceTracker, EvidenceDecision, EvidenceConfidence, EvidenceTrackerImpl } from '../orchestration/WorkflowOrchestrator.js';

export interface EnhancedValidationConfig {
  truthScoreThreshold: number; // ≥0.95 required
  maxCorrectionCycles: number; // Circuit breaker protection
 ...
 */
export class EnhancedADWValidationOrchestrator extends EventEmitter {
  private qualityGates: ADWQualityGates;
  private config: EnhancedValidationConfig;
  private validationResults: Map<string, ValidationResult> = new Map();
  private truthScoringMetrics: TruthScoringMetrics;
  private circuitBreakerState: 'CLOSED' | 'OPEN' | 'HALF_OPEN' = 'CLOSED';
  private correctionCycleCount = 0;
  ...
  private initializeTruthScoringMetrics(): void {
   ...
   */
  async executeEnhancedValidation(workflowId: string): Promise<TruthScoringMetrics> {
    ...
    try {
      ...
      const evidenceTracker = new EvidenceTrackerImpl();
   ...
   */
  private async executeGate2Requirements(workflowId: string, evidence: EvidenceTracker): Promise<void> {
    ...
    const execution = await this.qualityGates.executeGate(
      'gate-2-requirements',
      workflowId,
      evidence,
   ...
   */
  private async executeGate3Architecture(workflowId: string, evidence: EvidenceTracker): Promise<void> {
   ...
   */
  private async executeGate4ADRCompliance(workflowId: string, evidence: EvidenceTracker): Promise<void> {
   ...
   */
  private async executeGate5DeploymentReadiness(workflowId: string, evidence: EvidenceTracker): Promise<void> {
   ...
   */
  private async executeGate6EvidenceMetaValidation(workflowId: string, evidence: EvidenceTracker): Promise<void> {
   ...
   */
  private async gatherRequirementsEvidence(): Promise<EvidenceDecision> {
    ...
    return {
      ...
      confidence: EvidenceConfidence.SOLID, // 0.85
      evidence: [
   ...
   */
  private async gatherArchitectureEvidence(): Promise<EvidenceDecision> {
    return {
   ...
   */
  private async performCrossADRAnalysis(): Promise<EvidenceDecision> {
    return {
   ...
   */
  private async gatherDeploymentEvidence(): Promise<EvidenceDecision> {
    return {
   ...
   */
  private async performAdversarialReview(execution: GateExecution, challenges: string[]): Promise<void> {
    const adversarialChallenges: AdversarialChallenge[] = [];

    for (const challenge of challenges) {
      // Simulate adversarial challenge analysis
      const challengeResult: AdversarialChallenge = {
   ...
   */
  private classifyEvidenceQuality(confidence: number): EvidenceConfidence {
    if (confidence >= 0.85) return EvidenceConfidence.SOLID;
    if (confidence >= 0.55) return EvidenceConfidence.SOFT;
    if (confidence >= 0.25) return EvidenceConfidence.SHAKY;
   ...
   */
  private calculateEvidenceCoverage(evidence: EvidenceTracker): number {
   ...
   */
  private async calculateFinalTruthScore(workflowId: string): Promise<void> {
  ...
  private calculateOverallEvidenceCoverage(): number {
    const results = Array.from(this.validationResults.values());
    const solidEvidence = results.filter(r => r.evidenceQuality === EvidenceConfidence.SOLID).length;
  ...
  private calculateInvestigationCompleteness(): number {
    const results = Array.from(this.validationResults.values());
   ...
   */
  private processGateExecution(execution: GateExecution): void {
  ...
  private handleInvestigationRequired(event: any): void {
    this.emit('investigation_triggered', event);
  }

  private handleGateSequenceFailure(event: any): void {
   ...
   */
  private async triggerAutoRollback(workflowId: string, reason: string): Promise<void> {
