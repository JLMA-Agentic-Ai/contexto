...
exports.HealthCircuitBreaker = void 0;
const events_1 = require("events");
class HealthCircuitBreaker extends events_1.EventEmitter {
    config;
     ...
     */
    registerComponent(componentId, healthChecks, circuitConfig, failoverConfig) {
     ...
     */
    async executeRequest(componentId, operation, fallback) {
        ...
        try {
            ...
            const responseTime = Date.now() - startTime;
            // Record failure
            this.recordFailure(componentId, responseTime, error);
     ...
     */
    async triggerFailover(componentId, reason) {
     ...
     */
    executeMemoryHealthCheck(healthCheck) {
     ...
     */
    async executeDiskHealthCheck(healthCheck) {
     ...
     */
    recordSuccess(componentId, responseTime) {
     ...
     */
    recordFailure(componentId, responseTime, error) {
        const counts = this.requestCounts.get(componentId);
        if (counts) {
            counts.failure++;
        }
     ...
     */
    shouldAttemptReset(componentId) {
        const config = this.circuitConfigs.get(componentId);
     ...
     */
    evaluateAlertCondition(condition, metrics) {
    ...
    calculateOverallHealth() {
        ...
        if (unhealthyCount > total * 0.5)
            return 'unhealthy';
        if (unhealthyCount > 0 || degradedCount > total * 0.3)
            return 'degraded';
        return 'healthy';
    }
    /**
    ...
    calculatePerformanceTrends() {
        ...
        for (const [componentId, history] of this.componentMetrics.entries()) {
            ...
            const responseTimeTrend = this.calculateMetricTrend(history.map(m => ({ timestamp: m.timestamp, value: m.responseTime })));
            const errorRateTrend = this.calculateMetricTrend(history.map(m => ({ timestamp: m.timestamp, value: m.errorRate })));
            trends.push({
                componentId,
                metric: 'responseTime',
     ...
     * Find healthy backup component
     */
    async findHealthyBackup(primaryComponentId) {
...
exports.HealthCircuitBreaker = HealthCircuitBreaker;
//# sourceMappingURL=health-circuit-breaker.js.map