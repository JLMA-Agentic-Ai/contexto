...
exports.ADWEvidenceTracker = void 0;
const events_1 = require("events");
class ADWEvidenceTracker extends events_1.EventEmitter {
    config;
    evidence = new Map();
     ...
     */
    async challengeEvidence(evidenceId, challengerId, reason, counterEvidence, proposedLevel) {
     ...
     */
    async createDecisionTree(rootDecision, evidenceIds, tags = []) {
     ...
     */
    generateEvidenceReport(workflowId) {
     ...
     * Validate evidence level based on sources
     */
    validateEvidenceLevel(evidence) {
        const sourceReliability = evidence.sources.reduce((sum, source) => sum + source.reliability, 0) / evidence.sources.length;
        const sourceRelevance = evidence.sources.reduce((sum, source) => sum + source.relevance, 0) / evidence.sources.length;
     ...
     */
    calculateTreeConfidence(nodes) {
        const evidenceNodes = nodes.filter(n => n.type === 'evidence');
        const weightedSum = evidenceNodes.reduce((sum, node) => sum + (node.confidence * node.weight), 0);
        const totalWeight = evidenceNodes.reduce((sum, node) => sum + node.weight, 0);
...
exports.ADWEvidenceTracker = ADWEvidenceTracker;
/**
...
class ConfidenceCalculator {
    ...
    calculateWithChallenges(evidence) {
    ...
    calculateOverall(evidenceList) {
        ...
        const totalWeight = evidenceList.reduce((sum, evidence) => sum + this.getEvidenceWeight(evidence), 0);
        return totalWeight > 0 ? weightedSum / totalWeight : 0;
    }
...
class ValidationEngine {
    async validateThroughGate(evidence, gate) {
    ...
    async evaluateCriterion(evidence, criterion) {
    ...
    evaluateBuildStatus(evidence) {
        // Simulate build status check
        const buildSuccess = evidence.sources.some(s => s.type === 'test-results' && s.content.includes('success'));
        return {
            passed: buildSuccess,
    ...
    evaluateTestCoverage(evidence) {
        // Simulate test coverage check
        const coverageSource = evidence.sources.find(s => s.type === 'test-results' && s.content.includes('coverage'));
    ...
    generateGateRecommendations(gate, score, failures) {
        const recommendations = [];
...
//# sourceMappingURL=adw-evidence-tracker.js.map